...
bot.on('message', message => {
    ...
    if (msg.startsWith(prefix + 'HELP')) { // We're also going to use a seperate JSON file, so we need to call it.
        ...
        if (msg === `${prefix}HELP`) { // If they only type this, lets ONLY show the commands for regular users
            ...
            embed.setFooter(`Currently showing user commands. Commands are not case sensitive! To view another group do ${prefix}help [group / command]`)
            ...
            if (groupFound != '') { // If a group is found, run this statement.
                ...
                embed.setDescription(`**${commandsFound} commands found** - <> means required, [] means optional`)
								embed.setThumbnail("upload.wikimedia.org/wikipedia/commons/thumb/8/8d/Flag_of_the_Communist_Party_of_Vietnam.svg/2000px-Flag_of_the_Communist_Party_of_Vietnam.svg.png")
		...
		if(msg.split(/\s+|\./).includes('gay')||msg.split(/\s+|\./).includes('gey')){
			message.author.send("Homosexuality is not allowed in Soviet Russia! Off to the gulag with you!")
		...
		if(msg === prefix + "NICOISGAY"){
				...
				for(var i = 0; i < 3;i++){
					...
					message.channel.send("ur sister mister");
					message.channel.send("ur granny tranny");
					message.channel.send("ur grandpap a trap");
					message.channel.send("family tree LGBT");
					message.channel.send("ur super mega colossal tripple decker gay");
				}
		}
		if(msg === prefix + "MAN"){
      message.delete();
      message.channel.send("https://www.marxists.org/archive/marx/works/download/pdf/Manifesto.pdf")
		...
		if(msg.startsWith(prefix + "NOHOMO")){
			message.delete();
		...
		if(msg === prefix + "DICE"){
		 ...
		 const embed = new Discord.RichEmbed()
				 ...
				 message.channel.send({embed: {
						 color: 0x1D82B6,
						 description: `${message.author} you rolled a ${rawGuess}!`
				 }})
